...
import { createInstructor, getCourses, getInstructors, removeInstructorFromCourse } from "../../api/marketApi.js";
...
export default function MarketInstructorsPage() {
  ...
  const activeInstructors = instructors.filter((instructor) =>
    instructor.status === "ACTIVE" && instructor.name.toLowerCase().includes(search.trim().toLowerCase())
  );
  ...
  return (
    <section className="mx-auto max-w-7xl">
      ...
      <div className="mt-3 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"><div className="overflow-x-auto"><table className="w-full min-w-220 text-left text-sm"><thead className="bg-blue-50 text-xs uppercase tracking-wide"><tr><th className="px-5 py-3">Instructor</th><th className="px-5 py-3">Email</th><th className="px-5 py-3">Assigned courses</th><th className="px-5 py-3">Action</th></tr></thead><tbody>{activeInstructors.map((instructor) => <tr key={instructor.id} className="border-t border-slate-100"><td className="px-5 py-3 font-bold">{instructor.name}</td><td className="px-5 py-3 text-slate-500">{instructor.email}</td><td className="px-5 py-3"><div className="flex min-w-80 flex-col gap-2">{instructor.assignedCourses?.length ? instructor.assignedCourses.map((course) => <div key={course.id} className="flex items-center justify-between gap-3 rounded-lg bg-slate-50 px-3 py-2"><span className="font-semibold">{course.courseCode} - {course.title}</span><button type="button" onClick={() => removeAssignment(instructor, course)} className="shrink-0 rounded-md border border-red-200 px-2 py-1 text-xs font-bold text-red-700 hover:bg-red-50">Remove</button></div>) : <span className="text-slate-500">No courses assigned</span>}</div></td><td className="px-5 py-3"><button type="button" onClick={() => setInstructorToAssign(instructor)} className="rounded-lg bg-[#07529b] px-3 py-1.5 text-xs font-bold text-white">Assign to course</button></td></tr>)}{activeInstructors.length === 0 && <tr><td colSpan="4" className="px-5 py-6 text-center text-slate-500">No active instructors match your search.</td></tr>}</tbody></table></div></div>
