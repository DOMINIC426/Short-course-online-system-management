...
const MARKETING_OFFICER_LINKS = [
  { to: "/market/dashboard", label: "Dashboard", icon: LayoutDashboard },
...
function SidebarLinks({ onLinkClick }) {
  return (
    <nav className="flex-1 space-y-1 px-3 py-4">
      {MARKETING_OFFICER_LINKS.map((link) => {
...
export default function MarketingOfficerLayout() {
  ...
  // Only MARKETING_OFFICER role can access this layout
    ...
    <div className="flex min-h-screen bg-slate-50">
      ...
      <aside className="hidden w-64 flex-shrink-0 flex-col border-r border-slate-200 bg-white sm:flex">
        <Link to="/" className="flex items-center gap-2 border-b border-slate-200 px-6 py-5">
          ...
          <span className="text-sm font-semibold text-slate-900">Marketing Portal</span>
        </Link>
        <SidebarLinks />
