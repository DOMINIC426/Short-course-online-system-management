...
const INSTRUCTOR_LINKS = [
  ...
  { to: "/instructor/progress", label: "Course Progress", icon: TrendingUp },
  { to: "/instructor/venue", label: "Change Venue", icon: MapPin },
  { to: "/instructor/certificates", label: "Certificate Eligibility", icon: Award },
...
export default function InstructorLayout() {
  ...
  // Fetch /api/v1/instructor/me on mount
  useEffect(() => {
    let isMounted = true;

    async function fetchInstructorProfile() {
      if (!user || String(user.role || "").toUpperCase() !== "INSTRUCTOR") return;
      setProfileLoading(true);
      try {
        const response = await api.get("/api/v1/instructor/me");
        if (isMounted) {
          setInstructorProfile(response.data);
        }
      } catch (err) {
        console.error("Failed to load instructor profile header:", getApiErrorMessage(err));
      } finally {
  ...
  const activeProfile = instructorProfile || user;

  const displayName = activeProfile?.firstName
    ? `${activeProfile.firstName} ${activeProfile.lastName || ""}`.trim()
    : activeProfile?.email || "Instructor";
      ...
      <aside className="hidden w-[290px] flex-shrink-0 flex-col bg-[#0b4d94] text-white sm:flex">
        <div className="flex items-center gap-3 border-b border-white/10 px-5 py-5">
          ...
          <div className="leading-tight">
            ...
            <p className="text-[11px] text-white/75">Instructor Portal</p>
          </div>
        </div>
                ...
                <div className="absolute right-0 top-full z-50 mt-3 w-56 rounded-2xl border border-slate-200 bg-white p-2 shadow-xl">
                  <div className="flex items-center gap-3 px-2 py-2">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-[11px] text-slate-500">{activeProfile?.email}</p>
                    </div>
                  </div>
                  ...
                  <Link
                    ...
                    onClick={() => setIsUserMenuOpen(false)}
                    className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100"
                  >
                  ...
                  <button
                    ...
                    className="mt-1 flex w-full items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-rose-50 hover:text-rose-600"
                  >
