...
import { LayoutDashboard, Megaphone, Menu, X, LogOut } from "lucide-react";

const INSTRUCTOR_LINKS = [
  { to: "/instructor/dashboard", label: "My courses", icon: LayoutDashboard },
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium transition ${
                isActive ? "bg-udom-primary/10 text-udom-primary" : "text-slate-600 hover:bg-slate-100"
              }`
            }
          >
    ...
    <div className="flex min-h-screen bg-slate-50">
      ...
      <div className="flex flex-1 flex-col">
        <header className="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3 sm:hidden">
          <button onClick={() => setIsDrawerOpen(true)} className="p-2" aria-label="Open menu">
